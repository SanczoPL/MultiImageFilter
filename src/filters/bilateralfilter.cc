...
Filter::BilateralFilter::BilateralFilter(QJsonObject const &a_config)
...
void Filter::BilateralFilter::process(cv::Mat &a_src)
{
  cv::Mat dst = a_src.clone();
  cv::bilateralFilter(dst, a_src, m_diameter, m_sigmaColor, m_sigmaSpace, m_borderType);
}
